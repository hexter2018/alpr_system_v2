...
export async function updateTriggerZone(cameraId, points, zoneType = 'polygon') {
  const res = await fetch(`${API_BASE}/api/cameras/${cameraId}/trigger-zone`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function updateCameraStatus(cameraId, fps) {
  const res = await fetch(`${API_BASE}/api/cameras/${cameraId}/status`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function getHealthMetrics(hours = 1, metricType = null, metricName = null) {
  ...
  const res = await fetch(`${API_BASE}/api/health/metrics?${params}`)
  if (!res.ok) throw new Error('Failed to load metrics')
  return res.json()
}

export async function recordHeartbeat(cameraId, fps) {
  const res = await fetch(`${API_BASE}/api/health/heartbeat/${cameraId}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ fps })
  })
...
export async function searchPlates(filters = {}) {
  ...
  const res = await fetch(`${API_BASE}/api/search/plates?${params}`)
  if (!res.ok) throw new Error('Search failed')
  return res.json()
}

export async function getEvidence(readId) {
  const res = await fetch(`${API_BASE}/api/search/evidence/${readId}`)
